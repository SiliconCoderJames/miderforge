...
- ~~记忆一致性检查~~ ✅ v1 已完成（2026-09-09）：**确定性候选对检测 + 人工裁决**。`findContradictionCandidates` 用字符三元组 Dice 相似度（中文友好、免分词）找出事实类记忆中"高度相似但不相同"（0.35≤dice≤0.95）的对；记忆视图「🔍 矛盾扫描」按钮人工裁决归档哪条。LLM 语义裁决为后续增强：只对候选对批量提问，成本可控。
