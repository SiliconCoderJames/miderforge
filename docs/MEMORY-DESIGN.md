...
| L2 | 50 条 active | 超出部分单语句降权至 0.2（非删除，冷数据可被 L3 检索到） |
...
- **跨层预取**：任务开始时按目标预取相关技能摘要（与渐进披露平衡）；
- **淘汰策略升级**：L2 的 FIFO 降权升级为评分制（importance × 时间衰减）；
- **记忆一致性检查**：周期性扫描 L3 中语义互相矛盾的条目对，提示用户裁决。
